#!/usr/bin/env python3
"""Batch V4PH inference with hash-locked H placement and GAME MIDI_P."""
...
def require_runtime_hashes(runtime, singer_root):
    actual = {"train_v4ph.py": sha256_file(runtime / "train_v4ph.py")}
    for name, expected in EXPECTED_RUNTIME_HASHES.items():
        ...
        if archived_hash != expected or source_hash != expected:
            raise ValueError(
                f"V4PH runtime hash mismatch for {name}: "
...
def strict_checkpoint_metadata(
    ...
    if payload.get("run_state") != "complete" or int(payload.get("global_step", -1)) != 30000:
        raise ValueError("V4PH checkpoint is not the completed 30k final")
    metadata = payload.get("v4ph_training")
    if not isinstance(metadata, dict) or metadata.get("schema") != CHECKPOINT_SCHEMA:
        raise ValueError("V4PH checkpoint lacks authoritative training metadata")
    expected_values = {
    ...
    for key, expected in expected_values.items():
        if metadata.get(key) != expected:
            raise ValueError(f"V4PH metadata {key} mismatch: {metadata.get(key)} != {expected}")
    ...
    for key, expected in expected_contract.items():
        if contract.get(key) != expected:
            raise ValueError(f"V4PH H/PUL contract mismatch for {key}")
...
def build_model(
    ...
    if not isinstance(state, dict):
        raise ValueError("V4PH checkpoint lacks ema_model_state_dict")
    state = {
    ...
    if tuple(p_weight.shape) != (257, 128) or not torch.equal(
        ...
        raise ValueError("V4PH P embedding shape/PAD row is invalid")
...
def placement_audit(rendered, regions):
    ...
    return {
        "schema": "aisvc.v4ph-placement-audit.v1",
...
def synthesize(
    ...
    if rs != 44100 or ms != 44100:
        raise ValueError("V4PH evaluation requires 44.1kHz A/B WAVs")
    ...
    if tuple(p_classes.shape) != (1, total_frames) or (p_classes == 256).any():
        raise AssertionError("GAME emitted invalid V4PH model classes")
    ...
    if torch.count_nonzero(midi[:, :ref_frames]) != 0 or tuple(midi.shape) != (
        ...
        raise AssertionError("V4PH prompt MIDI masking failed")
    if not torch.equal(policy.smoothMelody_MIDIFuzzDisturb(midi), midi):
        raise AssertionError("V4PH precomputed MIDI transport is not tensor-identical")
    ...
    if len(dense_text) != total_frames or max(dense_text, default=0) > PUL_TOKEN:
        raise AssertionError("V4PH renderer emitted an invalid dense text tensor")
    ...
    audit.update(
        {
            ...
            "game": {
                "schema": "aisvc.v4ph-game-audit.v1",
...
def parse_args():
    parser = argparse.ArgumentParser(description="Batch V4PH evaluation runner")
...
def main():
    ...
    emit("loading_v4ph", checkpoint=str(checkpoint), groups=len(dataset_groups))
